...
class invariantOnNeuron:
    ...
    def setSafetyMargin(self, side: TYPES_OF_BOUNDS, safety_margin: float):
        assert safety_margin >= 0
    ...
    def reduceSafetyMargin(self, var, side, factor=2, maximal_factor=100, verbosity=0):
        ...
        if new_margin < SAFETY_FACTOR/maximal_factor:
            if verbosity > 0:
                ...
                if verbosity > 1:
                    print('Safety margin stays = ', current_margin)
            return False

        self.list_of_neurons[var].setSafetyMargin(side, new_margin)
        if verbosity > 0:
            print('Safety margin for variable ', var, ' side ', side, ' adjusted to ', new_margin)
        if new_margin < self.minimal_safety_margin:
            self.minimal_safety_margin = new_margin
            if verbosity > 1:
                print('Minimal safety margin adjusted to ', new_margin)
        ...
        if (not status) and adjust_epsilons:
            epsilon_adjusted = \
                self.layer_interpolant_candidate.strengthenEpsilons(out_of_bounds_inputs, difference_dict,
                                                                    ...
                                                                    number_of_epsilons=number_of_epsilons_to_adjust,
                                                                    adjust_safety_margin=True)
            # TODO: add adjust_safety_margin as an additional method argument
            if verbosity > 1:
