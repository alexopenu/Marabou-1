...
from basicLevelStatistics import *
...
#
#         if len(good_set):
#             self.recomputeStatistics(good_set, two_sided)
#
#
#     def recomputeStatistics(self, good_set=[], two_sided=True):
#
#         if not len(good_set):
#             self.statistics_computed = False
#             self.layer_size = 0
...
#             self.range = []
#         else:
#             self.sample_matrix = np.array(good_set)
...
#
#         sns.distplot([output[i] for output in self.sample_matrix],label='Variable'+str(i))
#         plt.legend()
#         plt.show()
#
...
class invariantOnNeuron:
    ...
    def __init__(self, layer = -1, var = -1, participates_in_invariant = True, tight_bounds = True,
                 ...
                 basic_statistics: basicLevelStatistics = None):
    ...
    def loadFromBasicStatistics(self, layer: int, var: int, basic_statistics: basicLevelStatistics,
...
class layerInterpolateCandidate:
    ...
    def loadFromBasicStatiatics(self, basic_statistics: basicLevelStatistics, layer = -1,
...
class MarabouNNetMCMH:
    ...
    def __init__(self, network_filename: str, property_filename: str, layer=-1):
        ...
        self.basic_statistics = basicLevelStatistics(sample_values_list = [], epsilon=self.epsilon)
        ...
        self.output_equations = self.marabou_nnet.property.get_exec_output_equations()
    ...
    def setOriginalPropertyLists(self):
        if self.marabou_nnet.property.mixed_properties_present() or self.marabou_nnet.property.ws_properties_present():
            ...
            print(self.marabou_nnet.property.properties_list)
            sys.exit(1)
    ...
    def initiateVerificationProcess(self, layer = -1, N=5000, compute_loose_offsets='range',
        ...
        self.basic_statistics.recomputeStatistics(self.good_set, two_sided=True)
...
class advanced_mcmh_statistics:
    ...
    def computeStatistics(self):
        ...
        assert self.sample_matrix

        self.layerSize = len(self.sample_matrix[0])

        self.layerMinimums = [min(self.sample_matrix[:,var]) for var in range(self.layerSize)]
        self.layerMaximums = [max(self.sample_matrix[:, var]) for var in range(self.layerSize)]
    ...
    def estimateStatisticsForVariable(self,var=0):
        ...
        outputs = sorted(self.sample_matrix[:,var])
...
if True:
    ...
    mcmh_object.prepareForMarabouCandidateVerification(network_filename1,network_filename2,output_property_file,
                                                       input_property_file, network_filename_disjunct=output_property_file)
    ...
    mcmh_object.checkConjunction(total_trials=1000,individual_sample=1000,number_of_epsilons_to_adjust=5,verbosity=2)
